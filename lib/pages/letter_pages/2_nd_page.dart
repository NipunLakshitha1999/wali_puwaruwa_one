                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      IconButton(icon: Icon(Icons.layers_clear), onPressed: (){
                        this.setState(() {
                          int clickcount = storage.getItem("letter_two_click_count");
                          if(clickcount == null){
                            clickcount=1;
                          }else{
                          ...
                          storage.setItem("letter_two_click_count", clickcount);
                          int count=storage.getItem("letter_two_incorrect");
                          if(count == null){
                            count=1;
                          }else{
                            count=count+1;
                          }
                          storage.setItem("letter_two_incorrect", count);
                          points.clear();
                        });
                      }),
                      IconButton(icon: Icon(Icons.check_box,color: selectedColor,), onPressed: () async{
                        ...
                        var response = await http.Client().post(url,body: json.encode({"user_name":storage.getItem("user_name"),"correct_count":1,"incorrect_count":1,"letter_no":2,"status":1}));
                        int clickcount = storage.getItem("letter_two_click_count");
                        if(clickcount == null){
                          clickcount=1;
                        }else{
                        ...
                        storage.setItem("letter_two_click_count", clickcount);
                        print(clickcount);
                        int count=storage.getItem("letter_two_correct");
                        if(count == null){
                          count=1;
                        }else{
                          count=count+1;
                        }
                        storage.setItem("letter_two_correct",count);
